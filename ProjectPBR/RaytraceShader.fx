...
struct Ray
{
	float3 A;
	float3 B;
	float3 GetPoint() { return A; }
	float3 GetDirection() { return B; }

};

const float3 LeftBottom = float3(-1.0f, -1.0f,1.0f);
const float3 Horizontal = float3(16.0f, 0.0f,1.0f);
const float3 Vertical = float3(0.0f, 9.0f, 1.0f);
...
RTInput VS(VSInput Input)
{
    ...
    Output.Ray = Output.Position.xyz;
...
PSInput RTWriter(RTInput Input)
{
    ...
    Output.Ray = float4(Input.Ray, 1.0f);
...
Ray GetRay(int u, int v)
{
    ...
    ray.A = ViewPosition;
    ray.B = LeftBottom + (PixelOffset.x * Horizontal) + (PixelOffset.y * Vertical) - ViewPosition;

    return ray;
}

float4 Lambertian(float3 Vec)
{
    return float4(1.0f, 1.0f, 1.0f, 1.0f);
}

float4 Color(Ray ray, float2 UV)
{
	//float3 Sample;
	//Sample = RayBuffer.Sample(SampleState, UV).rgb;

 //   ray = GetRay(UV.x, UV.y);
...
PSFinal DeferredRayPS(RTInput Input)
{
    PSFinal Output = (PSFinal) 0;
    Ray ray;
    
    float4 RayColor = Color(ray, Input.UV);

 //   Output.Color = RayColor;
...
technique11 RayTech
{
    ...
    {
        SetVertexShader(CompileShader(vs_5_0, DeferredVS()));
        SetPixelShader(CompileShader(ps_5_0, DeferredRayPS()));
    }
};